 ...
 * @revised Michael Bekos
 *
 */
public class PlanarGraphTriangulator {

	private PlanarGraphTriangulator() {}

	/**
	 * Makes a given planar graph biconnected (maintaining planarity).
	 * 
	 * It is not optimized for minimum number of new edges because
	 * this function is intended as prestep for graph triangulation.
	 * 
	 * @param graph a planar graph
	 * @return the additional edges as a List
	 * @throws Exception when the input graph is not planar
	 */
	public static List<Edge> makeBiconnectedPlanar(Graph graph) throws Exception
	{
		int n = graph.N();
	 ...
	 * 
	 * It is not optimized for minimum number of new edges because this
	 * function is intended as prestep for graph triangulation.
	 * 
	 * @param graph a planar graph
	 * @param plan a planar embedding of the given graph
	 * @param removePlanInsertedEdges if true, removes the double edges inserted by the given planar embedding
	 * @return the inserted edges as a List
	 */
	public static List<Edge> makeGraphBiconnectedPlanar(Graph graph, PlanarInformation plan, boolean removePlanInsertedEdges)
	{
		List<Edge> insertedEdges = new ArrayList<Edge>();
		...
		{
			System.out.println("Node active: " + curNode.toString());
			if (nmap.getBool(curNode))
			{
				...
				for (Edge curEdge = plan.cyclicNextEdge(preEdge); curEdge != edgeToStopAt; curEdge = plan.cyclicNextEdge(curEdge))
				{
					...
					// get the two next nodes of the two different biconnected components outer face
					...
					// create a new edge and insert it at the correct place (and it's reverse)
					Edge newEdge = graph.createEdge(source, plan.getReverse(preEdge), target, curEdge, Graph.BEFORE, Graph.AFTER);
					Edge newEdgeRev = graph.createEdge(target, plan.getReverse(curEdge), source, preEdge, Graph.AFTER, Graph.BEFORE);
	 ...
	 * @param plan The graph's corresponding embedding
	 */
	 ...
	 */
	public static List<Edge> triangulatePlanarGraph(Graph graph)
	{
		...
		List<Edge> insertedEdges = makeGraphBiconnectedPlanar(graph, plan, true);
	 ...
	 * 
	 * @param curFace the face to triangulate
	 */
	private static void triangulateFace(Face curFace, Graph graph, List<Edge> insertedEdges)
	{
		...
		// jump to second edge
		ec.next();
		// for all edges get source, add to helper list, and find node with minimum degree
		for (; ec.ok(); ec.next())
		{
		...
		// check v1 for more than 2 face neighbors first mark v1's neighbors
		...
		if (hasMoreThan2FaceNeighbors)
		{
			...
			for (MyNode cur = vjNode.next; cur != v1Node.prev; cur = cur.next) {
			...
			for (MyNode cur = v1Node.next.next; cur != v1Node.prev; cur = cur.next) {
				System.out.println("nodes " + v1Node.node + " " + cur.node);
