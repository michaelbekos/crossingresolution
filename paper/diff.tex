...
%DIF DEL submitted.tex   Fri Jul 26 08:45:28 2019
%DIF ADD journal.tex     Mon Aug  5 14:50:50 2019
...
%DIF -------
\usepackage{amsmath,amssymb,adjustbox} %DIF > 
%DIF -------
...
\title[Drawings of Graphs with High Crossing Resolution]{A Heuristic Approach towards Drawings of Graphs with High Crossing Resolution%DIF < 
\DIFdelbegin %DIFDELCMD < \thanks{A preliminary version of this work has appeared in the Proceedings of the 26th International Symposium on Graph Drawing and Network Visualization (GD 2018).
%DIFDELCMD < This work is supported by the DFG grant Ka812/17-1}%%%
\DIFdelend }
...
In 2017, we participated to the contest\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{DBLP:conf/gd/DevannyKLR17} }\hspace{0pt}%DIFAUXCMD
...
It is far from clear that the wining team was the team  ``\texttt{CoffeeVM}''\DIFdelbegin \DIFdel{; details for their approach can be found in ~\mbox{%DIFAUXCMD
\cite{DBLP:journals/corr/abs-1807-09483}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{. Notably,  their approach is in principle very similar to the one that we present in this paper. In particular, they also adopt the vertex movement paradigm on a set of a randomly chosen samples. Once they find a sample position, which improves the crossing resolution of the drawing, then they focus on a region close to this position. In this region, they are examining more sample positions, hoping that they will find one that will further improve the crossing resolution of the drawing; for details refer to~\mbox{%DIFAUXCMD
\cite{DBLP:conf/gd/DemelDMRW18}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFaddend Our team ``\texttt{TuebingenMidnight}''  \DIFdelbegin \DIFdel{miserably performed }\DIFdelend \DIFaddbegin \DIFadd{performed miserably}\DIFaddend ; we actually took the third position out of a total of four teams that participated. As already mentioned, each team had only one hour to compute a layout for each of the challenge graphs, which in year 2017 were in total $16$. In Table~\ref{tab:gdContest2017}, we additionally provide results that were achieved by the algorithm presented in this paper in two settings; within the time limit of the contest (see column ``Time restricted''), as well as  \DIFdelbegin \DIFdel{our best }\DIFdelend results, which were achieved without any time limitations (see column ``\DIFdelbegin \DIFdel{Our best}\DIFdelend \DIFaddbegin \DIFadd{No-time restriction}\DIFaddend '').
...
In 2018, we participated again to the Graph Drawing contest under the name ``\texttt{T\"ubingenColdShower}''\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{DBLP:conf/gd/DevannyKLR18} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend (using an implementation of the algorithm presented in this paper). The winning team of the Graph Drawing contest 2017 also participated under the name ``\texttt{CoffeeVM+}''. Table~\ref{tab:gdContest2018} summarizes the results, which are also available at \texttt{www.graphdrawing.de/contest2018/results.html}. 
...
Our algorithm is presented in detail in Section~\ref{sec:algorithm} and is experimentally evaluated against the ones by Argyriou et al.~\cite{DBLP:journals/cj/ArgyriouBS13} and by Huang et al.~\cite{DBLP:journals/vlc/HuangEHL13} in Section~\ref{sec:experiments}, where we also discuss our insights from this project. \DIFaddbegin \DIFadd{In Section~\ref{sec:samples} we present sample drawings produced by our algorithm. }\DIFaddend We conclude in Section~\ref{sec:conclusions} with a discussion of useful observations and insights that we obtained while working on this project.
...
In this section, we describe our heuristic for obtaining drawings with high crossing resolution. The input of our heuristic consists of a graph $G$ and an initial drawing $\Gamma_0$ of $G$ with crossing resolution $c(\Gamma_0)$. We assume that no two edges of graph $G$ overlap in drawing $\Gamma_0$, i.e., $c(\Gamma_0)>0$. A circular drawing or a drawing obtained by applying a force-directed algorithm on graph $G$ clearly meets this precondition. \DIFaddbegin \DIFadd{The output of our heuristic will be a (not necessarily grid) drawing $\Gamma$ with crossing resolution $c(\Gamma)$, such that $c(\Gamma) \geq c(\Gamma_0)$. In Section~\ref{ssec:variants}, we discuss how one can easily modify our heuristic so that $\Gamma$ is necessarily a grid drawing, if  the input drawing $\Gamma_0$ is a grid drawing, as well.  
}\DIFaddend 

Our algorithm is iterative and at each iteration performs some operations that are mainly based on randomization. At the $i$-th iteration, we assume that we have computed a drawing $\Gamma_{i-1}$ of graph $G$ \DIFdelbegin \DIFdel{of crossing resolution $c(\Gamma_{i-1}) \geq c(\Gamma_0)$. In other words, we assume, as }\DIFdelend \DIFaddbegin \DIFadd{with crossing resolution $c(\Gamma_{i-1})$. As }\DIFaddend an invariant for our algorithm, \DIFaddbegin \DIFadd{we further assume that for all values of $k$ with $0 \leq k \leq i-1$ the following condition holds %DIF > 
%DIF > 
}$$\DIFadd{c(\Gamma_{k-1}) \leq c(\Gamma_k).}$$ 
%DIF > 
\DIFadd{In other words, we assume }\DIFaddend that the crossing resolution cannot be decreased at some iteration of our algorithm (so, in a sense, our algorithm resembles probabilistic hill climbing approaches)\DIFaddbegin \DIFadd{, which also implies that $c(\Gamma_{i-1}) \geq c(\Gamma_{0})$}\DIFaddend . We proceed by choosing a vertex of $\Gamma_{i-1}$ arbitrarily at random based on the content of the so-called \emph{vertex-pool}, which may contain:
...
In general, aesthetically pleasant drawings of graphs are usually the result of compromising between different aesthetic criteria. Towards this direction, we discuss in this section interesting variants of our algorithm, which are motivated by the following observation that we made while working on this project (see Section~\ref{sec:conclusions}): Drawings that are optimised only in terms of the crossing resolution tend to have bad \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio (that is, the ratio of the longest to the shortest edge is large) and poor angular resolution (that is, the angles formed by adjacent edges are small). The former seems to be a consequence of the fact that drawings with good crossing resolution tend to be quite demanding in area. For the latter, observe that if in a drawing all edges are either almost horizontally or almost vertically drawn, such that only ``horizontal'' and ``vertical'' edges cross, then the crossing resolution of this drawing is arbitrarily close to $90^\circ$, while its angular resolution is arbitrarily close to~$0^\circ$.

% ==================================================================
\DIFdelbegin %DIFDELCMD < \myparagraph{Aspect ratio}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \myparagraph{Edge-length ratio}
\DIFaddend % ==================================================================
%
Formally, the \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio of a drawing is the ratio of the length of its longest edge to the length of its shortest edge\DIFaddbegin \DIFadd{; see, e.g.,~\mbox{%DIFAUXCMD
\cite{DBLP:journals/tcs/LazardLL19}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Sometimes it is also used as a measure of the area of non-grid drawings. It was easy to instruct our algorithm to prevent producing drawings with \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio either higher than the one of the starting layout or higher than a given input value. What we simply had to do was to reject candidate positions, which violate this precondition.
...
In this subsection, we report the results of our expertimental evaluation for the non-planar Rome graphs. Note that we did not impose any grid constraint on our algorithms. Our results are summarized in Figures~\ref{fig:cr-res}, \DIFdelbegin \DIFdel{\ref{fig:an-res} and~\ref{fig:to-res}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:to-res} and~\ref{fig:an-res}}\DIFaddend .

\myparagraph{Crossing resolution} Our results for the crossing resolution are summarized in Fig.~\ref{fig:cr-res}. Here, each algorithm was adjusted to maximize exclusively the crossing resolution (i.e., by ignoring the drawing's angular resolution). It is immediate to see that \DIFdelbegin \DIFdel{our algorithm outperforms all other ones in terms of }\DIFdelend \DIFaddbegin \DIFadd{on average }\DIFaddend the crossing resolution of the \DIFdelbegin \DIFdel{produced drawings , when we do }\DIFdelend \DIFaddbegin \DIFadd{drawings obtained by your algorithm, which does }\DIFaddend not impose any restriction on the \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio of the computed drawings\DIFaddbegin \DIFadd{, is considerably larger than the crossing resolutions of the remaining algorithms}\DIFaddend ; refer to the solid-black curve, denoted as \emph{Unrestricted}, in Fig.~\ref{fig:cr-res-1}. The variant of our algorithm, which does not violate the \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio of the initial layout, leads to drawings with slightly smaller crossing resolution; refer to the solid-gray curve, denoted as \emph{\DIFdelbegin \DIFdel{AR-restricted}\DIFdelend \DIFaddbegin \DIFadd{ELR-restricted}\DIFaddend }, in Fig.~\ref{fig:cr-res-1}. Finally, the two force-directed algorithms seem to produce drawings with worse crossing resolution; refer to the dotted-gray and dotted-black curves of Fig.~\ref{fig:cr-res-1} (by Argyriou et al.\ and by Huang et al., respectively).
...
While our unrestricted algorithm produces drawings with \DIFdelbegin \DIFdel{better crossing resolution }\DIFdelend \DIFaddbegin \DIFadd{larger crossing resolution on average}\DIFaddend , this comes at a cost of drastically increased \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio (see Fig.~\ref{fig:cr-res-2}), which, however, is still better that the corresponding \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio of the drawings produced by the algorithm of Argyriou et al. For the latter algorithm, it seems that the forces due to the angles formed at the crossings outperform the corresponding spring forces, which try to keep the lengths of the edges short. Going back to our unrestricted algorithm, its behavior is up to a certain degree expected, mainly due to the fact that there is no control on the lengths of the edges (as it is, e.g., in the case of the force-directed algorithm of Huang et al.~\cite{DBLP:journals/vlc/HuangEHL13}, whose \DIFdelbegin \DIFdel{sping forces inpose stong }\DIFdelend \DIFaddbegin \DIFadd{spring forces impose strong }\DIFaddend restrictions on the edge lengths). On the other hand, the \DIFaddbegin \DIFadd{edge-length ratio of the drawings obtained by the }\DIFaddend restricted variant of our algorithm, which does not allow the \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio to increase, \DIFdelbegin \DIFdel{has more or less comparable performance (in terms of aspect ratio ) as the one }\DIFdelend \DIFaddbegin \DIFadd{is very close to the edge-length ratio of the drawings obtained by the algorithm }\DIFaddend of Huang et al. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Note that the latter algorithm is the one yielding drawings that on average have the lowest edge-length ratio. }\DIFaddend Regarding the number of crossings, the restricted variant of our algorithm and the force-directed algorithm of Huang et al.\ yield drawings with comparable number of crossings\DIFdelbegin \DIFdel{, which at the same time is significantly smaller than the number of crossings produced by }\DIFdelend \DIFaddbegin \DIFadd{. On the other hand, }\DIFaddend the two other algorithms \DIFaddbegin \DIFadd{of our experiment produce drawings that on average have more crossings}\DIFaddend ; see Fig.~\ref{fig:cr-res-3}.
...
}\DIFdelend %DIF > Note, however, that both variants of our algorithm as well as the force-directed algorithm by Argyriou et al.\ tend to produce drawings of the same total resolution for larger graphs with a small difference in our favor.
...
\myparagraph{Angular resolution} We conclude the analysis of our experimental evaluation with the results for the angular resolution; see Fig.~\ref{fig:an-res}. Here, each algorithm was adjusted  to maximize only the angular resolution (i.e., by ignoring the drawing's crossing resolution). \DIFdelbegin \DIFdel{A notable observation is that, for small graphs the best results are achieved by the algorithm }\DIFdelend \DIFaddbegin \DIFadd{Our unrestricted algorithm yields drawings with better angular resolution than the corresponding ones produced by our restricted variant, and by the algorithms }\DIFaddend by Argyriou et al.\DIFdelbegin \DIFdel{, while for medium-size graphs by our unrestricted algorithm}\DIFdelend \DIFaddbegin \DIFadd{\ and by Huang et al.}\DIFaddend ; see Fig.~\ref{fig:an-res-1}. For \DIFdelbegin \DIFdel{large graphs , the two algorithms tend to have the same performance}\DIFdelend \DIFaddbegin \DIFadd{graphs with 90 or more vertices, however, our unrestricted algorithms and the algorithm by Argyriou et al.\ tend to produce drawings with the same angular resolution on average}\DIFaddend . The restricted variant of our algorithm yields drawings with slightly worse angular resolution. The algorithm by Huang et al.\ is outperformed by all algorithms of the experiment.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > 
The results for the \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio, the number of crossings and the required number of iterations are very similar with corresponding ones for the total resolution; see Figs.~\ref{fig:an-res-2}--\ref{fig:an-res-4}.\DIFdelbegin \DIFdel{This observation suggests that, for most of the graphs of our experiment, the angular resolution dominates the crossing resolution (and thus is the one defining the total resolution) in the constructed drawings, which explains the similarity in the reported results. The small differences result from the fact that the crossing resolution cannot be entirely neglected.
}\DIFdelend %DIF >  This observation suggests that, for most of the graphs of our experiment, the angular resolution is greater than the crossing resolution (and thus is the one defining the total resolution) in the constructed drawings, which explains the similarity in the reported results. The small differences result from the fact that the crossing resolution cannot be entirely neglected.
...
\DIFdelbegin %DIFDELCMD < \label{app:samples}
%DIFDELCMD < %%%
...
In this paper, we introduced a new heuristic aiming to produce drawings of high crossing resolution, for which we also presented variants that take into account other common aesthetic criteria in Graph Drawing. Our experimental evaluation indicates that the new heuristic is competitive to the state of the art force-directed algorithms (even when restricted to a given maximum \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio). For the latter algorithms, our evaluation indicates that their performance is good, only when several aesthetic criteria are taken into account.

It also worth noting that while working on this project, we made some useful observations and obtained some interesting insights. In particular, there is a recent observation (also supported by experiments) that drawings, in which the crossing angles are large, are easy to read and understand. We observed that drawings that are optimized only in terms of the crossing angles might be arbitrarily bad and may have several undesired properties. In particular, in these drawings it was very common to have adjacent edges to run almost in parallel and vertices to be very close to each other (see, e.g., Fig.~\ref{fig:graph5-argyriou-cr}). Hence, their angular resolution and \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio were often poor. The additional restrictions that we imposed regarding the angular resolution and the \DIFdelbegin \DIFdel{aspect }\DIFdelend \DIFaddbegin \DIFadd{edge-length }\DIFaddend ratio helped in significantly improving the readability of the produced drawings, without loosing too much of their quality in terms of the crossing resolution.
...
\paragraph{Acknowledgments.} The authors would \DIFdelbegin \DIFdel{also }\DIFdelend like to thank Simon Wegendt and Jessica Wolz for implementing the first version of our prototype. \DIFaddbegin \DIFadd{This work is supported by the DFG grant Ka812/17-1. A preliminary version of this work has appeared in the Proceedings of the 26th International Symposium on Graph Drawing and Network Visualization (GD 2018).
}\DIFaddend 

\bibliographystyle{compj}
\begin{thebibliography}{99}

\bibitem{DBLP:journals/combinatorica/FraysseixPP90}
de Fraysseix, H., Pach, J., and Pollack, R. (1990) How to draw a planar graph
  on a grid.
\newblock {\em Combinatorica}, {\bf  10}, 41--51.

\bibitem{DBLP:conf/gd/GutwengerM98}
...
\bibitem{DBLP:journals/algorithmica/Kant96}
Kant, G. (1996) Drawing planar graphs using the canonical ordering.
\newblock {\em Algorithmica}, {\bf  16}, 4--32.

\bibitem{DBLP:journals/iwc/Purchase00}
Purchase, H.~C. (2000) Effective information visualisation: a study of graph
  drawing aesthetics and algorithms.
\newblock {\em Interacting with Computers}, {\bf  13}, 147--162.

\bibitem{DBLP:journals/algorithmica/EadesW94}
Eades, P. and Wormald, N.~C. (1994) Edge crossings in drawings of bipartite
  graphs.
\newblock {\em Algorithmica}, {\bf  11}, 379--403.

\bibitem{DBLP:journals/tsmc/SugiyamaTT81}
Sugiyama, K., Tagawa, S., and Toda, M. (1981) Methods for visual understanding
  of hierarchical system structures.
\newblock {\em {IEEE} Trans. Systems, Man, and Cybernetics}, {\bf  11},
  109--125.

\bibitem{DBLP:journals/congnum/Eades84}
Eades, P. (1984) A heuristic for graph drawing.
\newblock {\em Congressus Numerantium}, {\bf  42}, 149--160.

\bibitem{DBLP:journals/spe/FruchtermanR91}
Fruchterman, T. M.~J. and Reingold, E.~M. (1991) Graph drawing by
  force-directed placement.
\newblock {\em Softw., Pract. Exper.}, {\bf  21}, 1129--1164.

\bibitem{DBLP:books/ph/BattistaETT99}
...
\bibitem{DBLP:conf/dagstuhl/1999dg}
Kaufmann, M. and Wagner, D. (eds.) (2001) {\em Drawing Graphs, Methods and
  Models},  LNCS, {\bf2025}. Springer.

\bibitem{DBLP:reference/crc/2013gd}
Tamassia, R. (ed.) (2013) {\em Handbook on Graph Drawing and Visualization}.
  Chapman and Hall/CRC.

\bibitem{DBLP:conf/apvis/Huang07}
Huang, W. (2007) Using eye tracking to investigate graph layout effects.
\newblock In Hong, S. and Ma, K. (eds.), {\em {APVIS}},  pp. 97--100. {IEEE}
  Computer Society.

\bibitem{DBLP:journals/vlc/HuangEH14}
Huang, W., Eades, P., and Hong, S. (2014) Larger crossing angles make graphs
  easier to read.
\newblock {\em J. Vis. Lang. Comput.}, {\bf  25}, 452--465.

\bibitem{DBLP:conf/gd/Purchase97}
Purchase, H.~C. (1997) Which aesthetic has the greatest effect on human
  understanding?
\newblock In Battista, G.~D. (ed.), {\em {Graph Drawing}},  Lecture Notes in
  Computer Science, {\bf1353},  pp. 248--261. Springer.

\bibitem{Shonan2016}
Hong, S. and Tokuyama, T. (2016).
\newblock Algorithmics for beyond planar graphs.
\newblock NII Shonan Meeting Seminar 089.

\bibitem{Dagstuhl2016}
Kaufmann, M., Kobourov, S., Pach, J., and Hong, S. (2016).
\newblock Beyond planar graphs: Algorithmics and combinatorics.
\newblock Dagstuhl Seminar 16452.

\bibitem{SoCG2017}
Liotta, G. (2017).
\newblock Graph drawing beyond planarity: Some results and open problems.
\newblock SoCG Week, Invited talk.

\bibitem{DBLP:journals/corr/abs-1804-07257}
Didimo, W., Liotta, G., and Montecchiani, F. (2018) A survey on graph drawing
  beyond planarity.
\newblock {\em CoRR}, {\bf  abs/1804.07257}.

\bibitem{DBLP:journals/tcs/DidimoEL11}
Didimo, W., Eades, P., and Liotta, G. (2011) Drawing graphs with right angle
  crossings.
\newblock {\em Theor. Comput. Sci.}, {\bf  412}, 5156--5166.

\bibitem{DBLP:journals/jgaa/ArgyriouBS12}
Argyriou, E.~N., Bekos, M.~A., and Symvonis, A. (2012) The straight-line {RAC}
  drawing problem is {NP}-hard.
\newblock {\em J. Graph Algorithms Appl.}, {\bf  16}, 569--597.

\bibitem{DBLP:journals/cj/ArgyriouBS13}
Argyriou, E.~N., Bekos, M.~A., and Symvonis, A. (2013) Maximizing the total
  resolution of graphs.
\newblock {\em Comput. J.}, {\bf  56}, 887--900.

\bibitem{DBLP:journals/vlc/HuangEHL13}
Huang, W., Eades, P., Hong, S., and Lin, C. (2013) Improving multiple
  aesthetics produces better graph drawings.
\newblock {\em J. Vis. Lang. Comput.}, {\bf  24}, 262--272.

\DIFaddbegin \bibitem{DBLP:conf/sofsem/BlasiusRR17}
\DIFadd{Bl}{\DIFadd{\"{a}}}\DIFadd{sius, T., Radermacher, M., and Rutter, I. (2017) How to draw a
  planarization.
}\newblock \DIFadd{In Steffen, B., Baier, C., van~den Brand, M., Eder, J., Hinchey, M.,
  and Margaria, T. (eds.), }{\em {\DIFadd{SOFSEM}}}\DIFadd{,  LNCS, }{\bf10139}\DIFadd{,  pp. 295--308.
  Springer.
}

\bibitem{DBLP:conf/dac/FiducciaM82}
\DIFadd{Fiduccia, C.~M. and Mattheyses, R.~M. (1982) A linear-time heuristic for
  improving network partitions.
}\newblock \DIFadd{In Crabbe, J.~S., Radke, C.~E., and Ofek, H. (eds.), }{\em \DIFadd{Design
  Automation Conference, }{\DIFadd{DAC}}}\DIFadd{,  pp. 175--181. }{\DIFadd{ACM/IEEE}}\DIFadd{.
}

\bibitem{6771089}
{\DIFadd{Kernighan}}\DIFadd{, B.~W. and }{\DIFadd{Lin}}\DIFadd{, S. (1970) An efficient heuristic procedure for
  partitioning graphs.
}\newblock {\em \DIFadd{The Bell System Technical Journal}}\DIFadd{, }{\bf  \DIFadd{49}}\DIFadd{, 291--307.
}

\bibitem{DBLP:conf/alenex/RadermacherRRW18}
\DIFadd{Radermacher, M., Reichard, K., Rutter, I., and Wagner, D. (2018) A geometric
  heuristic for rectilinear crossing minimization.
}\newblock \DIFadd{In Pagh, R. and Venkatasubramanian, S. (eds.), }{\em \DIFadd{Workshop on
  Algorithm Engineering and Experiments }{\DIFadd{ALENEX}}}\DIFadd{,  pp. 129--138. }{\DIFadd{SIAM}}\DIFadd{.
}

\bibitem{DBLP:conf/wsdm/UganderB13}
\DIFadd{Ugander, J. and Backstrom, L. (2013) Balanced label propagation for
  partitioning massive graphs.
}\newblock \DIFadd{In Leonardi, S., Panconesi, A., Ferragina, P., and Gionis, A. (eds.),
  }{\em {\DIFadd{ACM}} \DIFadd{International Conference on Web Search and Data Mining}}\DIFadd{,  pp.
  507--516. }{\DIFadd{ACM}}\DIFadd{.
}

\DIFaddend \bibitem{DBLP:journals/networks/GoldschmidtT94}
Goldschmidt, O. and Takvorian, A. (1994) An efficient graph planarization
  two-phase heuristic.
\newblock {\em Networks}, {\bf  24}, 69--73.

\bibitem{DBLP:journals/siamcomp/FormannHHKLSWW93}
Formann, M., Hagerup, T., Haralambides, J., Kaufmann, M., Leighton, F.~T.,
  Symvonis, A., Welzl, E., and Woeginger, G.~J. (1993) Drawing graphs in the
  plane with high resolution.
\newblock {\em {SIAM} J. Comput.}, {\bf  22}, 1035--1052.

\bibitem{DBLP:journals/dam/EadesL13}
Eades, P. and Liotta, G. (2013) Right angle crossing graphs and 1-planarity.
\newblock {\em Discrete Applied Mathematics}, {\bf  161}, 961--969.

\bibitem{DBLP:journals/dam/BachmaierBHNR17}
Bachmaier, C., Brandenburg, F.~J., Hanauer, K., Neuwirth, D., and Reislhuber,
  J. (2017) Nic-planar graphs.
\newblock {\em Discrete Applied Mathematics}, {\bf  232}, 23--40.

\bibitem{DBLP:journals/tcs/BrandenburgDEKL16}
Brandenburg, F.~J., Didimo, W., Evans, W.~S., Kindermann, P., Liotta, G., and
  Montecchiani, F. (2016) Recognizing and drawing ic-planar graphs.
\newblock {\em Theor. Comput. Sci.}, {\bf  636}, 1--16.

\bibitem{DBLP:journals/tcs/BekosDLMM17}
Bekos, M.~A., Didimo, W., Liotta, G., Mehrabi, S., and Montecchiani, F. (2017)
  On {RAC} drawings of 1-planar graphs.
\newblock {\em Theor. Comput. Sci.}, {\bf  689}, 48--57.

\bibitem{DBLP:journals/jgaa/AngeliniCDFBKS11}
...
\newblock {\em J. Graph Algorithms Appl.}, {\bf  15}, 53--78.

\bibitem{DBLP:journals/comgeo/ArikushiFKMT12}
Arikushi, K., Fulek, R., Keszegh, B., Moric, F., and T{\'{o}}th, C.~D. (2012)
  Graphs that admit right angle crossing drawings.
\newblock {\em Comput. Geom.}, {\bf  45}, 169--177.

\bibitem{DBLP:journals/mst/GiacomoDLM11}
...
\newblock {\em Theory Comput. Syst.}, {\bf  49}, 565--575.

\bibitem{DBLP:conf/s-egc/AngeliniBDFHKLL11}
...
\bibitem{DBLP:journals/ipl/DidimoEL10}
Didimo, W., Eades, P., and Liotta, G. (2010) A characterization of complete
  bipartite {RAC} graphs.
\newblock {\em Inf. Process. Lett.}, {\bf  110}, 687--691.

\bibitem{DBLP:journals/algorithmica/GiacomoDEL14}
\DIFdelbegin \DIFdel{Giacomo}\DIFdelend \DIFaddbegin {\DIFadd{Di Giacomo}}\DIFaddend , E.\DIFdelbegin \DIFdel{~D.}\DIFdelend , Didimo, W., Eades, P., and Liotta, G. (2014) 2-layer right
  angle crossing drawings.
\newblock {\em Algorithmica}, {\bf  68}, 954--997.

\bibitem{DBLP:conf/wg/HongN15}
Hong, S. and Nagamochi, H. (2015) Testing full outer-2-planarity in linear
  time.
\newblock In Mayr, E.~W. (ed.), {\em {WG}},  LNCS, {\bf9224},  pp. 406--421.
  Springer.

\bibitem{DBLP:journals/cjtcs/DujmovicGMW11}
Dujmovic, V., Gudmundsson, J., Morin, P., and Wolle, T. (2011) Notes on large
  angle crossing graphs.
\newblock {\em Chicago J. Theor. Comput. Sci.}, {\bf  2011}.

\bibitem{DBLP:journals/siamdm/AckermanFT12}
Ackerman, E., Fulek, R., and T{\'{o}}th, C.~D. (2012) Graphs that admit
  polyline drawings with few crossing angles.
\newblock {\em {SIAM} J. Discrete Math.}, {\bf  26}, 305--320.

\bibitem{DBLP:conf/apvis/DidimoLR10}
Didimo, W., Liotta, G., and Romeo, S.~A. (2010) Graph visualization techniques
  for conceptual web site traffic analysis.
\newblock {\em {IEEE PacificVis}},  pp. 193--200. {IEEE} Computer Society.

\bibitem{DBLP:conf/gd/DidimoLR10}
Didimo, W., Liotta, G., and Romeo, S.~A. (2010) Topology-driven force-directed
  algorithms.
\newblock In Brandes, U. and Cornelsen, S. (eds.), {\em {Graph Drawing}},
  LNCS, {\bf6502},  pp. 165--176. Springer.

\bibitem{DBLP:conf/gd/NguyenEHH10}
Nguyen, Q.~H., Eades, P., Hong, S., and Huang, W. (2010) Large crossing angles
  in circular layouts.
\newblock In Brandes, U. and Cornelsen, S. (eds.), {\em {Graph Drawing}},
  LNCS, {\bf6502},  pp. 397--399. Springer.

\DIFdelbegin %DIFDELCMD < \bibitem{DBLP:journals/corr/abs-1807-09483}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem{DBLP:conf/gd/DevannyKLR17}
\DIFadd{Devanny, W.~E., Kindermann, P., L}{\DIFadd{\"{o}}}\DIFadd{ffler, M., and Rutter, I. (2017) Graph
  drawing contest report.
}\newblock \DIFadd{In Frati, F. and Ma, K. (eds.), }{\em \DIFadd{Graph Drawing and Network
  Visualization}}\DIFadd{,  LNCS, }{\bf10692}\DIFadd{,  pp. 575--582. Springer.
}

\bibitem{DBLP:conf/gd/DemelDMRW18}
\DIFaddend Demel, A., D{\"{u}}rrschnabel, D., Mchedlidze, T., Radermacher, M., and Wulf,
  L. (2018) A greedy heuristic for crossing-angle maximization.
\newblock \DIFaddbegin \DIFadd{In Biedl, T.~C. and Kerren, A. (eds.), }\DIFaddend {\em \DIFdelbegin \DIFdel{CoRR}\DIFdelend \DIFaddbegin \DIFadd{Graph Drawing and Network
  Visualization}\DIFaddend },  \DIFaddbegin \DIFadd{LNCS, }\DIFaddend {\DIFdelbegin %DIFDELCMD < \bf  %%%
\DIFdel{abs/1807.09483}\DIFdelend \DIFaddbegin \bf11282\DIFaddend }\DIFaddbegin \DIFadd{,  pp. 286--299. Springer.
}

\bibitem{DBLP:conf/gd/DevannyKLR18}
\DIFadd{Devanny, W.~E., Kindermann, P., L}{\DIFadd{\"{o}}}\DIFadd{ffler, M., and Rutter, I. (2018) Graph
  drawing contest report.
}\newblock \DIFadd{In Biedl, T.~C. and Kerren, A. (eds.), }{\em \DIFadd{Graph Drawing and Network
  Visualization}}\DIFadd{,  LNCS, }{\bf11282}\DIFadd{,  pp. 609--617. Springer}\DIFaddend .

\bibitem{DBLP:books/lib/BergCKO08}
de Berg, M., Cheong, O., van Kreveld, M.~J., and Overmars, M.~H. (2008) {\em
  Computational geometry: algorithms and applications, 3rd Edition}. Springer.

\DIFaddbegin \bibitem{DBLP:journals/tcs/LazardLL19}
\DIFadd{Lazard, S., Lenhart, W.~J., and Liotta, G. (2019) On the edge-length ratio of
  outerplanar graphs.
}\newblock {\em \DIFadd{Theor. Comput. Sci.}}\DIFadd{, }{\bf  \DIFadd{770}}\DIFadd{, 88--94.
}

\DIFaddend \bibitem{DBLP:books/sp/04/WieseE004}
Wiese, R., Eiglsperger, M., and Kaufmann, M. (2004) \emph{yFiles} -
  visualization and automatic layout of graphs.
\newblock {\em Graph Drawing Software},  pp. 173--191. Springer.

\bibitem{DBLP:reference/crc/BattistaD13}
{Di Battista}, G. and Didimo, W. (2013) Gdtoolkit.
\newblock In Tamassia, R. (ed.), {\em Handbook on Graph Drawing and
  Visualization.},  pp. 571--597. Chapman and Hall/CRC.
...
\begin{figure*}[h!]
	...
	(a)--(c)~the variant of our algorithm without restrictions on the \DIFdelbeginFL \DIFdelFL{aspect }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{edge-length }\DIFaddendFL ratio, 
	(d)--(f)~the variant of our algorithm forced to maintain the input \DIFdelbeginFL \DIFdelFL{aspect }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{edge-length }\DIFaddendFL ratio,
