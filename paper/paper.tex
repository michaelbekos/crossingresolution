...
Our algorithm is presented in detail in Section~\ref{sec:algorithm} and is experimentally evaluated against the ones of Huang et al.~\cite{DBLP:journals/vlc/HuangEHL13} and Argyriou et al.\ in Section~\ref{sec:experiments}. We conclude in Section~\ref{sec:conclusions} with open problems. In the appendix, we add more experimental results on grid restricted drawings, on more test sets and on the graphs from the graph drawing competition in 2017.
...
%
As a test set for our experiment, we used the non-planar Rome graphs~\cite{DBLP:reference/crc/BattistaD13}, which form a collection of around 8.100 benchmark graphs. For another test set (AT\&T graphs) refer to Appendix~\ref{app:north}. 
...
\end{axis}
\end{tikzpicture}}}
\caption{Illustration of our experimental results on the crossing resolution.}
\label{fig:cr-res}
\end{figure}
...
\end{axis}
\end{tikzpicture}}}
\caption{Illustration of our experimental results on the total resolution.}
\label{fig:to-res}
\end{figure} 
...
\end{axis}
\end{tikzpicture}}}
\caption{Illustration of our experimental results on the angular resolution.}
\label{fig:an-res}
\end{figure} 
...
\end{axis}
\end{tikzpicture}}}
\caption{Experimental results for the crossing resolution experiment on the AT\&T graph testset.}
...
\end{axis}
\end{tikzpicture}}}
\caption{Experimental results for the total resolution experiment on the AT\&T graph testset.}
...
\end{axis}
\end{tikzpicture}}}
\caption{Experimental results for the angular resolution experiment on the AT\&T graph testset.}
...
Our primary motivation for this work was our participation to the graph drawing contest in 2017\footnote{\texttt{http://www.graphdrawing.de/contest2017/results.html}} on the maximization of the crossing resolution where we miserably performed. Our approach for the contest in 2017 was a mixture of the two force directed algorithms by Huang et al. and Argyriou et al.

We give a comparison of our new approach to the performances of the clear winner ``CoffeeVM'' of last year's graph drawing contest and our previous team ``TuebingenMidnight'' in Table~\ref{tab:gdContest2017}. Note that in the contest the teams had only one hour to compute a layout for all 15 contest graphs, for our algorithm we provide results that were achieved with the same time restriction (see column ``Time restricted'') as well as our best result which were achieved without a strict time restriction (see column ``Our best'').

\begin{table}[htb]

\caption{Results for the Graph Drawing Contest Graphs.}
\centering
...
We can observe that for almost all graphs, our new approach achieves only slightly worse results than the ones of the last year's contest winners. On a few graphs (namely graphs 10 and 11) we even achieve better results. With a single exception (graph 4), we also outperform our results from last year.
If we neglect the time restriction, for all the graphs, the results  are (sometimes considerably) better than or about the same as last year's contest winner's. We can conclude that our new approach has good potential for the application in this year's graph drawing contest, however, we also note that more careful graph dependent parametrization will be needed to compute competitive solutions within the provided time.
%\begin{tabular}{c|ccc}
%Graph & Contest Winner & Our New Approach & T\"ubingen Midnight \\
%\hline 
%
%Graph &  8 & 9 & 10 & 11 & 12 & 13 & 14 & 15 \\ 
%Contest Winner & 84.95 & 59.885 & 20.978 & 46.684 & 36.47 & 25.456 & 33.52 & 20.512 \\ 
%Our New Approach & 89.28 & 88.2 & 23.72 & 72 & 35.86 & 33.68 & 43.08 & 29.18 \\ 
%\end{tabular} 
