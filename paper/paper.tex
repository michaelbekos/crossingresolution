...
So far, we have described the main idea of our iterative algorithm, which at each iteration step chooses uniformly at random a vertex from the vertex-pool of the current drawing to move, so to improve the crossing resolution. We have not described, however, how the chosen vertex is moved, i.e., how to compute its new position. To this end, let $v_i$ be the vertex that has been chosen from the vertex-pool of the drawing $\Gamma_{i-1}$ at the $i$-th iteration of our algorithm. 

\begin{itemize}
\item Description of the randomized algorithm.
\item Big graphs $\rightarrow$ focus on critical edges.
\item Mimics the human. 
